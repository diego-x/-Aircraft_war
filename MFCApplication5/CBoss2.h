...
public:
	...
	CBoss2();
	~CBoss2();
	CBoss2(int x, int y, int m_nMotion);
	BOOL Draw(CDC* pDC, BOOL bPause)
	{
		
		if ((rect1.right >= m_ptPos.x + 450 )&& nnn)
		{
			m_ptPos.x += 1;
			if (rect1.right <= m_ptPos.x + 450)
				nnn = 0;
			if (rand() % 98 == 0)
			{
				m_ptPos.x += 10;
	...
	CRect GetRect()
	{
		return CRect(CPoint(m_ptPos.x - 100, m_ptPos.y), CPoint(m_ptPos.x + 400, m_ptPos.y + 200));
	}
	...
	//BOOL Fired();
private:
	static const int ENEMY_HEIGHT = 436;
	static const int ENEMY_width = 570;
